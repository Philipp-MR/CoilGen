function coil_parts=parameterize_mesh(coil_parts,surface_is_cylinder)
...
coil_parts(part_ind).coil_mesh = mesh_parameterization_func( coil_parts(part_ind).coil_mesh );
% figure; plotMesh(parameterized_mesh, 'uefb');
%create a the 2D data set for fit
else
%Planarization of cylinder
point_coords=coil_parts(part_ind).coil_mesh.vertices;
...
planar_mesh=triangulation(coil_parts(part_ind).coil_mesh.faces',[u_coord;v_coord;0*u_coord]');
boundary_edges=freeBoundary(planar_mesh);
...
function  mesh = mesh_parameterization_func(mesh_in)
