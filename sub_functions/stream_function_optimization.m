...
cost_function = @(x) sum((reduced_sensitivity_matrix*x-target_field_single').^2)+tikonov_reg_factor*max(((red_current_density_mat_u*x).^2+(red_current_density_mat_v*x).^2+(red_current_density_mat_w*x).^2).^(1/2));
