...
% % local_opening_widhts.high=zeros(1,circular_resolution);
% % local_opening_widhts.low=zeros(1,circular_resolution);
% % for circ_ind=1:circular_resolution
% % local_opening_widhts.high(circ_ind)=calc_local_opening_gab(loop_group.loops(loop_ind),cut_shapes(loop_ind).high_cut,cut_width);
% % local_opening_widhts.low(circ_ind)=calc_local_opening_gab(loop_group.loops(loop_ind),cut_shapes(loop_ind).low_cut,cut_width);
% % end
% % % %create a circular opening cut
% % cut_shapes(loop_ind).high_cutshape=opening_circle.*repmat(local_opening_widhts.high/2,[2 1])+cut_shapes(loop_ind).high_cut.uv;
% % cut_shapes(loop_ind).low_cutshape=opening_circle.*repmat(local_opening_widhts.low/2,[2 1])+cut_shapes(loop_ind).low_cut.uv;
